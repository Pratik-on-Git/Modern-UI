...
const PillButton = ({ text, onClickHandler, icon, className = "" }) => {
  return (
    <button
      className={`relative flex items-center bg-transparent p-0 border-none cursor-pointer ${className}`}
    ...
    >
      ...
      >
        <defs>
          <linearGradient id="pillBtnGradient" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="#352ce8" />
        ...
        <path
          ...
          fill="url(#pillBtnGradient)"
        />
      </svg>
      <span className="relative z-10 w-full flex items-center justify-between px-[32px] py-0" style={{ width: '100%' }}>
